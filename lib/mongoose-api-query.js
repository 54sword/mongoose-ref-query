...
module.exports = exports = function apiQueryPlugin (schema, ops) {
      ...
      try {
          ...
          if (options.throwErrors )
              ...
              if ( cb )
                  ...
                  // TODO should return a query that always resolves to empty array ?
                  return null;
          }
      }
  ...
  schema.statics.apiQueryPrepare = function(expression, config) {
     ...
     config = _.extend({}, defaultQueryConfig, config || {} );
     ...
     function evalExpression(schema, expression) {
         ...
         function addBooleanExpression(path, expression) {
             projection[path] = convertToBoolean(expression.$in[0]);
         }

         for ( var key in expression ) {
             if ( ~['$and','$or'].indexOf(key) ) {
                 var projectionArray = projection[key] = [];
                 for ( var j  = 0, jj = expression[key].length; j < jj; j++ ) {
                     projectionArray.push( evalExpression(schema, expression[key][j]) );
                 }
             }
             else if ( key == "$text" ) {
                 projection[ key ] = expression[key];
             }
             else if ( key == "$elemMatch" ) {
                 projection[ key ] = evalExpression(schema, expression[key]);
             }
             else {
                 // path
                 var path = key,
                     ...
                     c_schema = schema,
                     c_path, refd_model;
                 while (( c_path = path_parts.shift() )) {

                     if ( /^\d+$/.test(c_path) ) continue; // should test if parent was array

                     if ( path_parts.length ) {
                         c_schema = c_schema.paths[c_path];
                         if ( ! c_schema ) throw new Error("invalid path");
                         ...
                         if (( refd_model = isReference(c_schema) )) {
                             // TODO all this should be wrapped in a closure
                             // ( in case of multiple references this breaks )
                             refd_model = connection.model(refd_model);
                             var reference_query = {};
                             reference_query[path_parts.join('.')] = expression[key];
                             var query = refd_model.apiQueryPrepare(reference_query, {ids_only: true});
                             var promise = function() {
                                 return query()
                                 .then(function(ids) {
                                     ids = ids.map(function(obj) { return obj._id; });
                                     projection[c_path] = { $in: ids };
                                 });
                             };
                         ...
                         if ( ! isArray(c_schema) )
                             throw new Error("querying subpath of non-array/mixed/reference");
                         ...
                         if ( ~Object.keys(expression[key]).indexOf('$size') ) {
                             projection[ key ] = expression[key];
                         }
                         else {
                             switch ( c_schema.constructor ) {
                                 case Types.Boolean:
                                     addBooleanExpression(path, expression[key]);
                                     break;
                                 case Types.String:
                                     addStringExpression(path, expression[key]);
                                     break;
                                 case Types.Number:
                                     addNumberExpression(path, expression[key]);
                                     break;
                                 case Types.Mixed:
                                     projection[ key ] = expression[key];
                                     break;
                                 ...
                                 case Types.DocumentArray:
                                     if ( c_schema.schema ) {
                                         projection[ key ] = evalExpression(c_schema.schema, expression[key]);
                                     } else {
                                         switch ( c_schema.options.type[0] && Types[c_schema.options.type[0].name] ) {
                                             ...
                                             case Types.Mixed:
                                                 ...
                                                 break;
                                             case undefined:
                                                 addArrayExpression(path, expression[key]);
                                                 break;
                                             default:
     ...
     return function() {
         ...
         return Q.all(promises).then(function() {
             ...
             query = model.find(projection, config.ids_only ? '_id' : undefined);
...
/**
 * Expects a http GET params object and returns AQT with an and operator at its root.
 */
    ...
    for ( var i in o ) {
        switch (i) {
            ...
            case "sort_by":
                ...
                break;
            case "populate":
                query.config.populate[i] = o[i].split(',');
                break;
            case "$text":
                var args = o[i].split(','), // TODO add support for commas
                    text_search = { $text: { $search : args[0] } };
                if ( args.length > 1 )
                    text_search.$language = args[1];
...
/* operators that take an array as argument ( this is not the mongo arity ) */
var array_operators = [
"in",
"nin",
"not",
