  ...
  schema.statics.apiQueryParams = function(parsedParams) {
    ...
    var parseSchemaForKey = function (schema, keyPrefix, lcKey, criteria) {
      ...
      if ((matches = lcKey.match(/(.+?)\.(.+)/))) {
          ...
          if ( subschema.constructor === Types.Array ||          // array of typeschemas ( including mixed /ofMixed.secret=x )
              ...
              if ( (refd_type = subschema.options.ref) ||                                                                 // reference
                  ...
                  refd_model.apiQueryParsed( TODO criterias , undefined, true).exec().then(function(ids) {
                    for ( var i = 0, ii = ids.length; i < ii ; i++ )
                        idPromise.push( ids[i] );
                    // TODO signal the resolvement
                  }).done();

                  addSearchParam({ $in : idPromise });
                  */
      ...
      if ( pathtype === Types.ObjectId ) {
          ...
          if ( ~criteria.operators.indexOf("size") ) {
              addSearchParam({$size: criteria.args[0]});
          }
          else {
              var elementsType;
      ...
      function addString() {
          var operator = criteria.operators[0] || null,
              ...
              text = ~criteria.operators.indexOf('text');
              regex = ci || ~criteria.operators.indexOf('regex');

          if ( text ) {
              if ( regex && options.throwErrors )
                  throw new Error("the operators 'text' and 'regex' can't be combined");
              if ( ! regex ) {
                  newParam = { $search : criteria.args[0] };
                  if ( criteria.args.length > 1 )
                      newParam.$language = criteria.args[1];
                  addSearchParam('$text', newParam);
                  return;
                  // doesn't seem it can be nested as subexpression
                  // criteria.args = criteria.args.map(function(s) { return { $text : { $search : s } }; });
              }
          }
      ...
      function addSearchParam(/* [key,] val */) {
          var key = ( arguments.length < 2 ) ? keyPrefix + lcKey : arguments[0],
              val = ( arguments.length < 2 ) ? arguments[0] : arguments[1];
