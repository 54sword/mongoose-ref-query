...
module.exports = exports = function refQueryPlugin (schema, opts) {
  ...
  schema.statics.refQueryPrepare = function(expression, config) {
     ...
     function prepareDependencyQuery(model, projection_path, path, expression) {
         ...
         _debug("\nREFERENCE_QUERY:");
         _debug(reference_query);
         var query = model.refQueryPrepare(reference_query, {ids_only: true}),
             trigger = function() {
                 return query()
                 .then(function(ids) {
                     ids = ids.data.map(function(obj) { return obj._id; });
                     var ncond = {};
     ...
     function prepareDependencyQueryBR(model, path, expression, referencingProp) {
         ...
         _debug("\nBackREFERENCE_QUERY :");
         _debug(reference_query);
         var query = model.refQueryPrepare(reference_query),
             trigger = function() {
                 return query()
                 .then(function(ids) {
                     _debug("\nBackREFERENCE returned with :");
                     _debug(ids);
                     ids = ids.data.map(function(obj) { return getValAtPath(referencingProp, obj); });
                     var ncond = {};
                     ncond._id = { $in: ids };
                     if (!projection.$and) projection.$and = [];
                     projection.$and.push(ncond);
             ...
             if ( ~["$and","$or"].indexOf(key) ) {
                 ...
                 while (( c_path = rest_of_path.shift() )) {
                     ...
                     if ( rest_of_path.length ) {
                         ...
                         if (c_schema.paths[c_path])
                             c_schema = c_schema.paths[c_path];
                         else if ((backref = options.backreferences[path_parts.slice(0,path_parts.length-rest_of_path.length).join(".")])) {
                             console.log("-- its a backreference!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
     ...
     return function() {
         ...
         return Q.all(promises).then(function() {
             ...
             _debug("\nquerying with: ");
             _debug( JSON.stringify(projection) );
             //*/
...
function getValAtPath(path, obj) {
